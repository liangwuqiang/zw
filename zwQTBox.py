...
def xtick2tim_code010(qx):
    ...
    for kss in qx.min_ksgns:
        # rx.min_knum, rx.min_ksgnWrk = int(kss), 'M'+kss  # 原来的，以下替换
        qx.min_knum, qx.min_ksgnWrk = int(kss), 'M' + kss
...
def xedit_zwXDat(df):
    ...
    clst=["open", "high", "low", "close", "volume", "xdiff", "z-open",
          "z-high", "z-low", "z-close", "z-xdiff", "ma5", "ma10", "ma20",
          "ma30", "v-ma5", "v-ma10", "v-ma20", "v-ma30"]
...
def stk_col_renLow(dat):
    ...
    dat =dat.rename(columns={'Open':'open', 'High':'high', 'Low':'low',
